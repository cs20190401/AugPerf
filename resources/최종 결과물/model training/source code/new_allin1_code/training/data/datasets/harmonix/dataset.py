...
class HarmonixDataset(DatasetBase):
  def __init__(
    ...
    if split == 'test':
      ...
      T_list = [self.load_features(self.track_ids[i]).shape[-2] for i in range(len(self.track_ids))]
      max_T = max(T_list) - self.latency if self.latency is not None else max(T_list)
      max_T = int(math.ceil(max_T/batch_size)*batch_size)
