...
class GTZANConverter(DatasetConverter):
  ...
  def __init__(
    ...
    *,
    ...
    track_id_underscore = self.track_id.replace('.', '_')

    self.df_beat = pd.read_csv(
      self.base_dir / 'beats' / f'gtzan_{track_id_underscore}.beats',
      names=['time', 'count'],
      delimiter='\t'
    )
    self.df_downbeat = self.df_beat[self.df_beat['count'] == 1]
...
class UnionConverter(DatasetConverter):
  ...
  def __init__(
    ...
    *,
    ...
    if name in ['Hainsworth']:
      annotation_path = self.base_dir / name / "beats" / f"{name.lower()}_{track_id}.beats"
    elif name in ['Ballroom', 'Carnatic', 'RWC']:
      annotation_path = self.base_dir / name / "beats" / f"{track_id}.beats"
    elif name in ['SMC']:
      annotation_path = self.base_dir / name / "beats" / f"{track_id.lower()}.beats"
    elif name in ['harmonix']:
      annotation_path = self.base_dir / name / "beats" / f"{track_id}.txt"
    elif name in ['GTZAN']:
      annotation_path = self.base_dir / name / "beats" / f"{name.lower()}_{track_id.replace('.','_')}.beats"

    if name != "SMC":
      delimiter = ',' if name == 'Carnatic' else '\t'
